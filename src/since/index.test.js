const { since, timeUnitSince, YEAR, MONTH, DAY, MINUTE, SECOND, WEEK } = require(".");

describe("timeUnitSince", () => {
    it("should throw with a non-number time", () => {
        expect(() => timeUnitSince("")).toThrow();
    });
    ...
    it("should throw with a non-number timeUnit", () => {
        expect(() => timeUnitSince(Date.now(), "string")).toThrow();
    });
...
describe("since", () => {
    it.each([
        ...
        expect(since(...args)).toStrictEqual(expected);
    });
});
